...
class TrefleDAO {
    ...
    suspend fun getImage(biljka: Biljka): Bitmap {
        return try {
            // Provjeri postoji li već BiljkaBitmap za ovu biljku
            val existingBitmap = withContext(Dispatchers.IO) {
                if (biljkaDao.existsBitmapByIdBiljka(biljka.id) == 1) {
                    biljkaDao.getBitmapById(biljka.id)
                } else {
                    null
                }
            }
            ...
            if (existingBitmap != null) {
                // Ako postoji, vrati tu bitmapu
                existingBitmap
            } else {
                // Inače traži sliku od servisa
                ...
                if (plantResponse.plant.image_url.isNotEmpty()) {
                    ...
                    // Ako nema slike u odgovoru servisa, baci iznimku ili vrati defaultnu bitmapu
                    defaultBitmap ?: throw IllegalStateException("Default bitmap not set")
                }
            }
        } catch (e: Exception) {
            // Uhvati bilo koju iznimku i logiraj je, zatim baci ili vrati defaultnu bitmapu
    ...
    suspend fun fixData(biljka: Biljka): Biljka {
        return try {
            ...
            if (plantResponse.plant.image_url.isNotEmpty()) {
                ...
                biljka.family = plantDetails.family.name
                            ...
                            if (plantDetail.plant.mainSpecies.flower.color != null){
                                biljke.add(
                                    Biljka(
                                        ...
                                        family = plantDetail.plant.family.name,
